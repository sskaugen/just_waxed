from Employee import Employee
class mathlogic():
    #constructor to hold the math logic
    def __init__(self):
        self.__weekpay = 0
        
    #Parametrs: self,Employee,Commison_rate,total_$_amount_services
    #returns: the total amount to be paid to the employee in commisson
    @staticmethod
    def total_made_commison_from_services(employee,total_amount_services):
        commison_rate_services = employee.get_commission_services()
        commison_to_pay_services = round(total_amount_services * commison_rate_services,2)
        return commison_to_pay_services
    
    #Parametrs: self,Employee,,total_$_amount_products
    #returns: the total amount to be paid to the employee in commisson for products
    @staticmethod
    def total_made_commissom_from_products(employee,total_amount_products):
        commison_rate_products = employee.get_commission_products()
        commison_to_pay_products = round(total_amount_products * commison_rate_products,2)
        return commison_to_pay_products
    
    #Parametrs: self,Employee,total_hours_worked
    #returns: the total to be paid for houly
    @staticmethod
    def total_from_weekly_pay(employee,hours_worked):
        total_hourly_to_pay = employee.get_hourly_rate() * hours_worked
        return total_hourly_to_pay
    
    #Parametrs: self,Employee,total_hours_worked,total_products,total_sevices
    #returns: the total to be paid to employee for weekely pay
    @staticmethod
    def final_weekly_pay(employee,hours_worked,total_products,_total_services):
        #calculate the total hourly pay
        total_hourly_to_pay = mathlogic.total_from_weekly_pay(employee,hours_worked)
        #calculate the total commisson Services
        total_commisson_services_made = mathlogic.total_made_commison_from_services(employee,_total_services)
        #calculate the total commisson Products
        total_commisson_products_made = mathlogic.total_made_commissom_from_products(employee,total_products)
        final_weekly_pay_to_employee = total_hourly_to_pay + total_commisson_services_made + total_commisson_products_made
        return final_weekly_pay_to_employee
    
    #want to come up with an algorthim / formula to determine how much the bussiness owner made after paying the employee
    #this will be for records as well as to determine profit margins are being kept
